...
\tikzset{
  ...
  slbl/.style={font=\tiny, text=VUgrey},
...
\begin{frame}{GNN Surrogates Across Application Domains}
  ...
  \visible<2->{{\normalsize Different domains, \textbf{the same computation}:
  message passing over node \textbf{and} edge features.\\[0.5ex]
  \textcolor{VUgoldDark}{\textbf{These predictions carry real consequences, so
...
\begin{frame}{Message Passing: Message $\to$ Aggregate $\to$ Update}
  ...
  \begin{tikzpicture}[scale=0.92, transform shape]
    ...
    \node[slbl, anchor=south] at (1.4,2.25) {\textbf{target node $v$ in its neighbourhood}};
    ...
    \draw[gedge] (r)--(u3);
    \node[font=\tiny, text=VUgrey!80, anchor=north, align=center] at (-1.3,-0.95)
    ...
    \visible<2->{
      ...
      \node[font=\tiny, text=VUgoldDark, anchor=south west] at (1.0,0.85) {$e_{u_1v}$};
      \node[stepbox, anchor=west] at (3.4,1.6)
        {$m_{uv} = \phi\big(h_u^{(\ell)},\, \textcolor{VUgoldDark}{e_{uv}}\big)$};
      \node[slbl, anchor=north west, text width=3.4cm] at (3.4,1.1)
        {\textcolor{VUgoldDark}{\textbf{1. Message:}} each neighbour $u$ sends
         its features, mixed with the \textcolor{VUgoldDark}{edge feature}
         $e_{uv}$.};
    }
    % ================= (c) aggregate =================
    \visible<3->{
      \node[stepbox, anchor=west, draw=aggcol, fill=aggcol!12] at (3.4,-0.3)
        {$\aggr{z_v = \textstyle\sum_{u\in\mathcal{N}(v)} m_{uv}}$};
      \node[slbl, anchor=north west, text width=3.4cm] at (3.4,-0.8)
        {\textbf{\aggr{2. Aggregate:}} a permutation-invariant sum over
         neighbours.};
    }
    % ================= (d) update / combine =================
    \visible<4->{
      \node[stepbox, anchor=west, draw=combcol, fill=combcol!12] at (3.4,-2.2)
        {$\comb{h_v^{(\ell+1)} = \sigma\big(W z_v + b\big)}$};
      \node[slbl, anchor=north west, text width=3.4cm] at (3.4,-2.7)
        {\textbf{\comb{3. Update}} (a.k.a.\ combine): learned linear map,
         then ReLU.};
    }

    % ================= (e) vectors on the right =================
    \visible<2->{
      \node[slbl, anchor=south] at (9.9,2.3) {\textbf{what node $v$ holds}};
      \foreach \x/\lab in {8.4/$m_{u_1v}$, 9.1/$m_{u_2v}$, 9.8/$m_{u_3v}$}{
        \foreach \i in {0,...,3} {\node[vec=VUgold!45] at (\x, 0.6+\i*0.24) {};}
        \node[font=\tiny, text=VUgrey, anchor=north] at (\x+0.09,0.55) {\lab};}
    }
    \visible<3->{
      \node[font=\large, text=aggcol] at (10.45,1.0) {$\Rightarrow$};
      \foreach \i in {0,...,3} {\node[vec=aggcol!45] at (11.0, 0.6+\i*0.24) {};}
      \node[font=\tiny, text=VUgrey, anchor=north] at (11.09,0.55) {$z_v$};
    }
    \visible<4->{
      \node[font=\large, text=combcol] at (10.45,-1.15) {$\Downarrow$};
      \foreach \i in {0,...,3} {\node[vec=combcol!40] at (11.0, -1.55+\i*0.24) {};}
      \node[font=\tiny, text=VUgrey, anchor=north] at (11.14,-1.6) {$h_v^{(\ell+1)}$};
      \node[slbl, anchor=east, text width=2.5cm, align=right] at (10.2,-1.15)
    ...
    \visible<5->{
      ...
      \node[font=\tiny, text=unsafered, anchor=east] at (-1.85,1.35) {layer 2};
    }
  \end{tikzpicture}

  \visible<5->{\vskip-0.2ex
  ...
  \refline{Gilmer et al., ICML 2017;\ \ Xu et al., ICLR 2019.}
\end{frame}
...
\begin{frame}{Graph Convolutional Network (GCN) and Graph Isomorphism Network with Edge Features (GINE)}
  ...
  {\small Same three stages as the previous slide:\ \
   \textcolor{VUgoldDark}{\textbf{message}} $\to$
   \aggr{\textbf{aggregate}} $\to$ \comb{\textbf{combine}} (update).\par}
  \vskip-0.3ex

  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.49\textwidth}
      \begin{block}{GCN [Kipf \& Welling, ICLR 2017]}
        \small
        \vspace{-3ex}
        \[
          H^{(\ell+1)} = \sigma\big(
    ...
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{block}{GINE [Hu et al., ICLR 2020]}
        \small
        \vspace{-3ex}
        \begin{align*}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.42\textwidth}
      ...
      \begin{tikzpicture}[scale=0.68]
        ...
        \draw[->,VUgoldDark,thick] (0,0)--(-0.4,0.9) node[font=\tiny, above]{$b_2$};
      ...
      \begin{alertblock}{This Work}
        Lift NNV's star-set engine from \textbf{vectors} to \textbf{graphs}:
        the \key{GNNV} module.
      \end{alertblock}
    \end{column}
  \end{columns}
...
\begin{frame}{GraphStar Reachability: How a Layer Transforms the Set}
  \centering
  A \textbf{linear map} $L$ hits the center \emph{and} every generator, so the
  GraphStar structure is preserved \good{exactly}:
  \vspace{-0.5ex}
  \[
    L\,\lb\langle C,\{B_i\},P\rangle\rb \;=\;
    \big\langle\, \aggr{LC},\ \{\comb{LB_i}\},\ P \,\big\rangle .
  \]
  \vspace{-1.5ex}
  {\small Aggregation $\tilde A$, gather $S$, scatter $R^\top$, weights and
  bias are all such maps.\par}
  \vskip0.8ex
...
\begin{frame}{GNNV Reachability Pipeline}
  \centering
  \vfill
  \includegraphics[width=0.88\linewidth]{figures/gnnv_pipeline_v4.pdf}
  \refline{Figure from our paper, Fig.~2.}
\end{frame}

\begin{frame}{Pipeline: GraphStar Construction to Specification Check}
  \centering
  \includegraphics[width=0.63\linewidth]{figures/gnnv_pipeline_v4.pdf}
  \vskip1ex
...
\begin{frame}{RQ3: Enclosure Tightness vs.\ Polynomial Zonotopes}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \visible<2->{\footnotesize GNNV stays tight where polynomial zonotopes
      accumulate error, at higher per-graph cost; \key{relax-star} recovers up
      to $10\times$.\par\smallskip
      \textit{Full per-$\epsilon$ tables are in the paper.}}
    \end{column}
  \end{columns}
  \takeaway{Up to 21.6\% more graphs verified than CORA's polynomial zonotopes.}
\end{frame}
  ...
  \begin{tikzpicture}[remember picture, overlay]
    ...
    \node[align=center, text=white] at (current page.center) {
      ...
      \begin{tabular}{cc}
        ...
        {\scriptsize\color{VUgold} paper (Springer)}
      \end{tabular}
    };
  \end{tikzpicture}
\end{frame}
