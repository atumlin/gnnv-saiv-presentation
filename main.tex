...
\begin{frame}{Message Passing: Message $\to$ Aggregate $\to$ Update}
  ...
  \begin{tikzpicture}[scale=0.9, transform shape]
    ...
    \node[font=\small, text=VUgrey, anchor=south] at (0.1,2.35) {\textbf{target node $v$ in its neighbourhood}};
    ...
    \visible<2->{
      \node[font=\small, text=VUgrey, anchor=south] at (10.15,2.35) {\textbf{what node $v$ holds}};
...
\begin{frame}{Two Message-Passing Architectures: GCN and GINE}
  ...
  \par\smallskip\noindent\hspace*{-0.24cm}%
  \begin{minipage}{\dimexpr\textwidth+0.48cm\relax}
  \begin{minipage}[t]{\dimexpr0.5\textwidth+0.19cm\relax}
      \begin{block}{\scriptsize Graph Convolutional Network (GCN)}
        \small
        \vspace{-3ex}
        \[
          H^{(\ell+1)} = \sigma\big(
    ...
    \end{minipage}\hfill
  \begin{minipage}[t]{\dimexpr0.5\textwidth+0.19cm\relax}
      \begin{block}{\scriptsize Graph Isomorphism Network with Edge Features (GINE)}
        \small
        \vspace{-3ex}
        \begin{align*}
