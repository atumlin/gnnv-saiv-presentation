...
\begin{frame}{Graph-Structured Inputs: Node and Edge Feature Matrices}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.52\textwidth}
      ...
      {\scriptsize A power grid mapped to a graph. Any domain maps in a similar
...
\begin{frame}{Message Passing: Message $\to$ Aggregate $\to$ Update}
  ...
  \begin{tikzpicture}[scale=0.9, transform shape]
    ...
    \node[font=\small, text=VUgrey, anchor=south] at (-0.25,2.35) {\textbf{target node $v$ in its neighbourhood}};
    ...
    \visible<2->{
      \node[font=\small, text=VUgrey, anchor=south] at (9.8,2.35) {\textbf{what node $v$ holds}};
...
\begin{frame}{Two Message-Passing Architectures: GCN and GINE}
  {\centering\small Same three stages as the previous slide:\ \
   \textbf{message} $\to$
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.49\textwidth}
      \begin{block}{\scriptsize Graph Convolutional Network}
        \footnotesize
        \vspace{0.4ex}
        \[
          H^{(\ell+1)} = \sigma\big(
    ...
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{block}{\scriptsize Graph Isomorphism Network with Edge Features}
        \footnotesize
        \vspace{-1.4ex}
        \begin{align*}
  ...
  {\centering\small Both share one shape: a \textbf{linear graph operation
  followed by ReLU}; only GINE carries edge features.\par}
...
\begin{frame}{Verification Gap: No Support for Edge-Aware Architectures}
  ...
  \begin{tabular}{@{}lcccc@{}}
    ...
    H\"ojny MILP (ICML 2024) & \good{\checkmark} & \bad{$\times$} & \good{\checkmark} & \bad{$\times$}\\
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.60\textwidth}
      ...
      {\scriptsize PF robustness vs.\ $\epsilon$, IEEE-24/-39/-118
       (\textcolor{safegreen}{verified}/\textcolor{unsafered}{violated}/%
...
\begin{frame}{Tool Availability: GNNV in NNV3 (MATLAB)}
  ...
  \begin{columns}[c,onlytextwidth]
    ...
    \begin{column}{0.28\textwidth}
      \centering
      \colorbox{white}{\includegraphics[width=2.0cm]{figures/qr_nnv.png}}\\[0.2ex]
      {\scriptsize NNV on GitHub}\\[1ex]
      \colorbox{white}{\includegraphics[width=2.0cm]{figures/qr_paper.png}}\\[0.2ex]
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    ...
    \node[align=center, text=white] at (current page.center) {
      ...
      \begin{tabular}{cc}
        \colorbox{white}{\includegraphics[width=1.55cm]{figures/qr_nnv.png}} &
        \colorbox{white}{\includegraphics[width=1.55cm]{figures/qr_paper.png}}\\[0.4ex]
...
\begin{frame}[plain,noframenumbering]
  \begin{tikzpicture}[remember picture, overlay]
    \fill[VUblack] (current page.north west) rectangle (current page.south east);
    \coordinate (L) at ([xshift=1.1cm]current page.west);
    \node[anchor=west, inner sep=0pt, text width=0.84\paperwidth, align=left] at ([yshift=0.3cm]L)
      {\usebeamerfont{title}\color{white}\bfseries\LARGE Backup Slides};
    \draw[VUgold, line width=1.6pt] ([yshift=-0.62cm]L) -- ([xshift=0.42\paperwidth, yshift=-0.62cm]L);
  \end{tikzpicture}
\end{frame}

\begin{frame}[noframenumbering]{GraphStar Reachability: How a Layer Transforms the Set}
  \centering
  A linear map transforms a GraphStar's center and every generator, so the
  structure is preserved \good{exactly}; every affine graph operation is lossless.
  \vskip1.2ex
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.49\textwidth}
      \begin{block}{GINE reachability (node $+$ edge)}
        \footnotesize
        \begin{itemize}\setlength{\itemsep}{0.5ex}\setlength{\topsep}{0pt}
          \item \textbf{Minkowski sum} fuses node $+$ edge:
                $S\,\lb\Theta_X\rb \oplus \lb\Theta_E\rb\,W_e$
          \item exact for star sets $\Rightarrow$ exact for \textbf{GraphStars};
                MLP ReLU $\to$ \key{approx-star}
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}[noframenumbering]{The Verified Models Are Accurate}
  \centering
  {\small Test MSE ($\times10^{-3}$) for PF and OPF across architectures and grid sizes.\par}
  \vskip0.8ex
  {\scriptsize\setlength{\tabcolsep}{10pt}\renewcommand{\arraystretch}{1.2}
  \begin{tabular}{llccc}
    \toprule
    Task & Architecture & IEEE-24 & IEEE-39 & IEEE-118\\
    \midrule
    PF  & GCN              & 51.9 & 48.8 & 46.2\\
        & GraphSAGE        & 22.7 & 22.6 & 12.9\\
        & GINE-Linear      & 1.92 & 2.91 & 5.36\\
        & \textbf{GINEConv} & \good{0.86} & \good{1.01} & \good{2.20}\\
    \midrule
    OPF & GCN              & 32.6 & 34.9 & 19.2\\
        & GraphSAGE        & 20.1 & 29.3 & 13.4\\
        & GINE-Linear      & 1.63 & 3.16 & 9.52\\
        & \textbf{GINEConv} & \good{0.69} & \good{0.99} & \good{2.29}\\
    \bottomrule
  \end{tabular}}
  \vskip1ex
  {\small The edge-aware \textbf{GINEConv} models we verify are also \good{by far the
  most accurate} (25--60$\times$ lower MSE than GCN).\par}
\end{frame}

\begin{frame}[noframenumbering]{relax-star: Precision--Speed Trade-off}
  \centering
  {\small Resolving a fraction $\rho$ of uncertain ReLUs with cheap interval bounds instead of LPs.
  Verified count and per-graph time (s/g).\par}
  \vskip1.2ex
  {\footnotesize\setlength{\tabcolsep}{7pt}\renewcommand{\arraystretch}{1.25}
  \begin{tabular}{ll cc cc cc}
    \toprule
    & & \multicolumn{2}{c}{\textbf{approx-star}} & \multicolumn{2}{c}{$\rho{=}0.8$} & \multicolumn{2}{c}{$\rho{=}0.9$}\\
    \cmidrule(lr){3-4}\cmidrule(lr){5-6}\cmidrule(lr){7-8}
    Dataset & $\epsilon$ & ver. & s/g & ver. & s/g & ver. & s/g\\
    \midrule
    ENZYMES     & .005 & \good{45/81}   & 8.00 & 44/81   & 1.79 & 44/81   & 0.99\\
    PROTEINS    & .010 & \good{152/167} & 3.98 & 151/167 & 0.90 & 151/167 & 0.49\\
    IEEE-24 CFA & .005 & \good{99/100}  & 0.36 & 99/100  & 0.08 & 99/100  & 0.04\\
    \bottomrule
  \end{tabular}}
  \vskip1.4ex
  {\small $\rho{=}0.9$ keeps nearly all verified graphs at \good{up to $\sim$9$\times$} speedup,
  retaining most of GNNV's advantage over CORA.\par}
\end{frame}

\begin{frame}[noframenumbering]{Soundness, Completeness, and Scope}
  \begin{itemize}\itemsep=1.3ex
    \item \textbf{Sound, not complete.} approx-star over-approximates ReLU, so GNNV
          \good{never misses a real violation}, but may return
          \textcolor{amber}{\textbf{unknown}} instead of a definitive safe/unsafe.
    \item \textbf{Guarantees are w.r.t.\ the trained model} and hold for \emph{all}
          perturbations inside the $\ell_\infty$ ball, not a finite sample.
    \item \textbf{Perturbations:} node and edge \emph{feature} perturbations ($\ell_\infty$).
          Structural perturbations (edge insertion/deletion) are future work.
    \item \textbf{Complexity:} worst-case exponential in the ReLU count; kept tractable
          by \key{$K$-hop} locality and \key{relax-star}.
  \end{itemize}
\end{frame}


\begin{frame}[noframenumbering]{Why GraphStar Is Tighter Than CORA (and Slower per Graph)}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.49\textwidth}
      \begin{block}{CORA: polynomial zonotopes}
        \footnotesize
        \begin{itemize}\setlength{\itemsep}{0.5ex}\setlength{\topsep}{0pt}
          \item ReLU via polynomial over-approximation
          \item error \bad{accumulates} layer by layer
          \item cheap per graph ($\sim$0.1--0.2\,s/g)
          \item loosens as $\epsilon$ grows $\Rightarrow$ more
                \textcolor{amber}{\textbf{unknown}}
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{block}{GNNV: GraphStar $+$ approx-star}
        \footnotesize
        \begin{itemize}\setlength{\itemsep}{0.5ex}\setlength{\topsep}{0pt}
          \item ReLU via \key{LP relaxation} (2 LPs/neuron)
          \item bounds stay \good{tight} across layers
          \item costlier per graph; \key{relax-star} up to $10\times$
          \item verifies where CORA's bounds overlap
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
  \vskip0.8ex
  {\centering\small Tighter enclosures verify \good{up to 21.6\% more graphs}; the
  cost is per-graph runtime, which relax-star recovers.\par}
\end{frame}

\begin{frame}[noframenumbering]{Why $K$-hop Subgraph Verification Is Sound}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.56\textwidth}
      In a $K$-layer message-passing GNN, information moves \textbf{one hop per
      layer}, so node $v$'s output depends only on its \key{$K$-hop
      neighbourhood}:
      \[ f_\theta(X,A,E)_v = f_\theta(X_v^{K}, A_v^{K}, E_v^{K})_v . \]
      Running reachability on the restricted subgraph GraphStars is therefore
      \good{exact} for $v$; over-approximation still comes only from ReLU.
      \smallskip

      On IEEE-118 (3-layer GINE), a typical 3-hop subgraph holds \textbf{15--35
      of 118} nodes, slashing the ReLU count and giving per-node queries in
      seconds.
    \end{column}
    \begin{column}{0.42\textwidth}
      \centering
      \begin{tikzpicture}[scale=0.9]
        \draw[VUgrey!40, thick, dashed] (0,0) circle (1.9);
        \draw[VUgold!55, very thick] (0,0) circle (1.05);
        % coordinates: v (target), u* (1-hop, inside K-hop), w* (beyond K-hop)
        \coordinate (v) at (0,0);
        \coordinate (u1) at (30:0.85); \coordinate (u2) at (150:0.85);
        \coordinate (u3) at (270:0.85);
        \coordinate (w1) at (85:1.62); \coordinate (w2) at (205:1.62);
        \coordinate (w3) at (330:1.62);
        % edges (behind the nodes)
        \draw[VUgrey, thick] (v)--(u1) (v)--(u2) (v)--(u3) (u1)--(u2);
        \draw[VUgrey!55, thick] (u1)--(w1) (u2)--(w2) (u3)--(w3);
        % nodes on top
        \foreach \n in {u1,u2,u3}
          \node[circle,fill=steelblue!35,draw=VUgrey,minimum size=4mm,inner sep=0pt] at (\n) {};
        \foreach \n in {w1,w2,w3}
          \node[circle,fill=VUgrey!15,draw=VUgrey,minimum size=3.4mm,inner sep=0pt] at (\n) {};
        \node[circle,fill=VUgold!55,draw=VUblack,thick,minimum size=5mm,inner sep=0pt,font=\scriptsize\bfseries] at (v) {$v$};
        \node[font=\tiny, text=VUgoldDark, fill=white, inner sep=0.6pt] at (-0.72,1.18) {$K$-hop};
        \node[font=\tiny, text=VUgrey!70] at (0,-2.15) {full graph ($N{=}118$)};
      \end{tikzpicture}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}[noframenumbering]{Where GNNV Sits Among GNN Verifiers}
  \centering
  {\scriptsize\setlength{\tabcolsep}{6pt}\renewcommand{\arraystretch}{1.35}
  \begin{tabular}{@{}lllll@{}}
    \toprule
    \textbf{Method} & \textbf{Approach} & \textbf{Arch.} & \textbf{Perturbations} & \textbf{Guarantee}\\
    \midrule
    CORA (TMLR'25)   & poly.\ zonotopes      & GCN            & node $+$ limited struct. & over-approx.\\
    H\"ojny (ICML'24) & MILP $+$ bounds       & ReLU MPNN      & node (topology-aware)    & exact\\
    Liu et al.\ (2025) & exact aggregation     & MPNN           & structural $+$ feature   & exact\\
    Wu et al.\ (2022) & unroll to FFN         & job scheduler  & node                     & exact\\
    \key{GNNV (ours)} & GraphStar $+$ approx-star & GCN, \textbf{GINE} & \textbf{node $+$ edge} $\ell_\infty$ & sound over-approx.\\
    \bottomrule
  \end{tabular}}
  \vskip1.4ex
  {\small Only GNNV verifies \textbf{edge-aware GINE} under joint node\,$+$\,edge
  feature perturbations. Exact methods do not scale to these; CORA is GCN-only.\par}
\end{frame}

\begin{frame}[noframenumbering]{The Robustness Cliff (IEEE-24 PF)}
  \centering
  {\small Extending node-only perturbations beyond the main study ($\epsilon\le10^{-2}$).\par}
  \vskip1.2ex
  {\footnotesize\setlength{\tabcolsep}{16pt}\renewcommand{\arraystretch}{1.3}
  \begin{tabular}{cccc}
    \toprule
    $\epsilon$ & Verified (\%) & Unknown (\%) & Time (s/graph)\\
    \midrule
    0.01 & \good{82.7} & 17.3  & 5.11\\
    0.02 & 54.5        & 43.9  & 8.01\\
    0.05 & \bad{2.2}   & 96.7  & 24.90\\
    0.10 & \bad{0.0}   & 100.0 & 72.66\\
    \bottomrule
  \end{tabular}}
  \vskip1.4ex
  {\small A sharp \textbf{cliff between $\epsilon=0.02$ and $0.05$}: beyond
  ${\sim}2\%$ of the normalized input range the model's voltage predictions
  become unverifiable (mostly \textcolor{amber}{\textbf{unknown}}, from
  reachable-set growth).\par}
\end{frame}

\begin{frame}[noframenumbering]{Training and Verification Setup}
  \begin{itemize}\itemsep=1ex
    \item \textbf{Trained in PyTorch Geometric}, exported to NNV (MATLAB) via
          \texttt{.mat} files.
    \item \textbf{PF / OPF} (node regression): \textbf{GINE};\ \
          \textbf{CFA / ENZYMES / PROTEINS} (graph classification): three-layer
          \textbf{GCN} (for CORA compatibility).
    \item All models use \textbf{ReLU}; Adam optimizer; best of \textbf{5 seeds}
          (val.\ MSE or test acc.); hidden dim \textbf{32} (64 for ENZYMES).
    \item Verification: GraphStar reachability with \key{approx-star} (2 LPs per
          crossing ReLU), \key{$K$-hop} subgraph extraction, optional
          \key{relax-star}.
    \item 100 graphs per perturbation level; CORA baseline run on GCN, its
          supported class.
  \end{itemize}
\end{frame}
